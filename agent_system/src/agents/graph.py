...
class BlogGenerationGraph:
    ...
    async def create_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            decide_next_action,
            {
                "generate": "generate",  # match whatever your react_agent returns
                END: END,
            },
        )
    ...
    async def compile_app(self):
        ...
        ### Removign the memroy saver for nwo (** TBD by ADMIN)
        # Get memory saver
        ...
        # # NEW – compile **without** any check-pointer
    ...
    async def run_blog_generation(
        ...
        # Configuration for LangGraph execution (Errror Part with Enahce memory)
        config = {
        ...
        ### skipping the meory saver for now ( To be changes very important)

        # # NEW – only keep execution limits
        # config = {
        #     "recursion_limit": 15,
        #     "max_concurrency": 4,
        # }
        ...
        try:
            ...
            if isinstance(final_state, dict) and "__end__" in final_state:
                final_graph_state = final_state["__end__"]
            elif isinstance(final_state, GraphState):
                final_graph_state = final_state
            # else:
            #     final_graph_state = final_state
            ...
            final_graph_state = GraphState(**final_state)
