...
class ContentExtractor:
        ...
        try:
            ...
            if self.include_links:
                for ref in soup.find_all("ref"):
                    link_data = {"url": ref.get("target", ""), "text": ref.text}
    ...
    def extract_from_crawler_result(
        self, crawler_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if not crawler_result.get("success", False) or "html" not in crawler_result:
            logger.warning(
                f"Invalid crawler result for URL: {crawler_result.get('url', 'unknown')}"
            )
    ...
    def extract_readability_metrics(self, text: str) -> Dict[str, float]:
        ...
        try:
            ...
            flesch_reading_ease = (
                ...
                - 84.6 * (syllable_count / max(1, word_count))
            )

            # Calculate Flesch-Kincaid Grade Level
            # Formula: 0.39 * (words/sentences) + 11.8 * (syllables/words) - 15.59
            flesch_kincaid_grade = (
                0.39 * (word_count / max(1, sentence_count))
                + 11.8 * (syllable_count / max(1, word_count))
                - 15.59
            )
