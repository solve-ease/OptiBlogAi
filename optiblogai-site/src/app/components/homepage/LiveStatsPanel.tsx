...
const LiveStatsPanel: React.FC = () => {
  ...
  const prevGithubDataStringRef = useRef("");

  // Create stats array from GitHub data
  useEffect(() => {
    if (!githubData?.repository) return;

    // convert data to string for comaprison
    const currentDataStrng = JSON.stringify(githubData.repository);

    if (currentDataStrng === prevGithubDataStringRef.current) return;

    prevGithubDataStringRef.current = currentDataStrng;
            ...
            <div className="flex items-center space-x-2 text-sm text-gray-500">
              {githubLoading ? (
                ...
                <>
                  ...
                  <span>
                    Live data from GitHub API • Last updated:{" "}
          ...
          {githubLoading && stats.length === 0
            ? // Loading skeleton
              Array.from({ length: 6 }).map((_, index) => (
                <Card key={index} className="group animate-pulse">
